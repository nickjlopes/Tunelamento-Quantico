...
np.savetxt('entropy-U{0:1d}-J{1:1d}-{2:1d}{3:1d}-python.txt'.format(int(U),
                                                                    int(J),
                                                                    M,
                                                                    P),
           np.c_[tt,von])
...
for t in tqdm(range(0,len(tt))):
  ...
  psit = np.zeros((dim), dtype=complex)
  vn = 0.

  for j in range(0,dim):
    psit = psit + ( np.dot(np.conj(evc[:,j]), psi0)*evc[:,j]
                   *np.exp(-1j*evl[j]*tt[t]) )
  for i in range(0,n+1):
    vn = vn - imeas(psit, i)*np.log2(imeas(psit, i))
